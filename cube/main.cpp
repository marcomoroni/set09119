...
int main()
{
	...
	for (int i = 0; i < dominoPiecesQuantity; i++)
	{
		...
		newRb.setMass(5.0f);
		newRb.scale(glm::vec3(1.0f, 1.0f, 1.0f));
		...
		newRb.setPos(glm::vec3(0.0f, 15.0f, 0.0f));
		newRb.setVel(glm::vec3(0.0f));
		newRb.setAngVel(glm::vec3(0.0f, 0.1f, 0.5f));
	...
	bool initalImpulseApplied = true;
