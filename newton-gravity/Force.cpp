...
** NEWTON GRAVITY
*/
glm::vec3 NewtonGravity::apply(float mass, const glm::vec3 &pos, const glm::vec3 &vel)
{
	float distance = glm::distance(m_b1->getPos(), m_b2->getPos());

	glm::vec3 direction = glm::normalize(m_b2->getPos() - m_b1->getPos());

	float power = m_g * (m_b1->getMass() * m_b2->getMass() / (distance * distance));
	
	// TEST make it look better
	if (distance < 1.0f) {
		//direction *= -1.0f;
		power *= 0.0f;
	}
